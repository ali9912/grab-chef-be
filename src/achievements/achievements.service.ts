...
import { AchievementEnum, Achievements } from './interfaces/achievement.interface';
...
export class AchievementsService {
    ...
    constructor(
        ...
        @InjectModel('Achievements') private readonly achievementsModel: Model<Achievements>
    ) { }

    async create(createAchievementDto: CreateAchievementDto) {
        const achievement = await this.achievementsModel.create(createAchievementDto)
        console.log(achievement)
        await achievement.save()
        return {
            achievement, message: "Achievement goal has been created."
        }
    }
    ...
    async checkForAchievements(userId: string) {
        ...
        console.log("sfddf", userId)
        if (!chef) {
            throw new HttpException("No chef found for achievements", HttpStatus.NOT_FOUND)
        }
        ...
        const filteredAchievements = achievements.filter(i => userAchievementIds.has(i._id))
        ...
        for (let i = 0; i < achievements.length; i++) {
            ...
            acheivement.conditions.forEach(element => {
                ...
                if (element.type === AchievementEnum.FIVE_STARS) {
                    ...
                    console.log("MATCHING THE FIVE STARS", isMatched, chef?.noOfFiveStars)
                }
                if (element.type === AchievementEnum.FOUR_STARS) {
                    isMatched = element.qty === chef.noOfFourStars
                    console.log("MATCHING THE FOUR STARS", isMatched, chef?.noOfFourStars)
                }
            })
        ...
        console.log("matchefd", matchedItems)
